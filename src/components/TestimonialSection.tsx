      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10">
          {TESTIMONIALS.map((testimonial) => (
            <div
              key={testimonial.id}
              className="bg-panel border border-beige-dark/40 p-8 sm:p-10 relative flex flex-col justify-between hover:shadow-xl transition-all duration-300 rounded-none"
            >
              {/* Quote Icon overlay */}
              <div className="absolute top-6 right-8 text-ink/10">
              ...
              <div>
                ...
                {/* Review Text */}
                <p className="font-sans text-ink/80 text-sm leading-relaxed italic font-light mb-6">
                  "{testimonial.text}"
                </p>
              </div>

              {/* Bottom part - User details */}
              <div className="flex items-center space-x-4 border-t border-beige-dark pt-6">
                {/* Avatar with initials */}
                <div className="w-11 h-11 bg-gold/10 border border-gold/20 text-gold rounded-full flex items-center justify-center font-serif text-sm font-bold">
                  {testimonial.initials}
                </div>
                
                {/* Details */}
                <div>
                  <h4 className="font-serif text-sm font-bold text-ink font-medium">
                    {testimonial.name}
                  </h4>
                  ...
                  <span className="inline-block mt-1.5 text-[10px] font-sans font-medium px-2 py-0.5 bg-beige-light border border-beige-dark/40 text-ink/70">
